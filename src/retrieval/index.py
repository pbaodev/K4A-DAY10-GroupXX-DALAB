...
class LocalEmbeddingIndex:
    ...
    def build(
        ...
        # Relative to the project so committed manifests do not leak or depend on a local absolute path.
        try:
            stored_persist_path = persist_path.resolve().relative_to(settings.paths.project_dir.resolve()).as_posix()
        except ValueError:
